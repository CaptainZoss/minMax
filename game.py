...
class GameBoard:
    ...
    def run(self): #the main method of the game   
        ...
        while black == None and white == None: #this loops until players enter a valid value and pick to play either black or white
            val = input ("Enter B to play Black and W to play White: ")
            if val == 'B' or val == 'b':
            ...
            if (turn == 'Black' and black == 'AI') or (turn == 'White' and white == 'AI'): #if it is the AI's turn
                if (self.firstMove):
                    if (turn == "Black"):
                        ...
                        print(turn + " move set " + str(posMoves))#print the possible moves
                        ...
                        if movecol > 1: self.whiteFirstmove.append([(moverow,movecol-1),0])
                    if (turn == "White"):
                        print(turn + " move set " + str(self.whiteFirstmove))#print the possible moves
                        move = random.choice(self.whiteFirstmove)
                        self.firstMove = False

                if move == None:
                    #move = self.alg.minimaxDecision(self.board, turn, 1)
                    if (turn == "White"): move = random.choice(allMoves)
                    else: move = self.alg.miniMaxAB(turn, 1)
                ...
                while move == None:
                    move = input ("Enter a valid move: ") #use format x1,y1 x2,y2 or x1,y1 0
...
gb = GameBoard()
gb.run()
evals += gb.alg.evaluations
bfac += (gb.alg.branchFactor/gb.alg.num)
cts += gb.alg.cuts
...
print ("avg eval "+ str(evals/5))
print ("avg bfac "+ str(bfac/5))
print ("avg cuts "+ str(cts/5))
